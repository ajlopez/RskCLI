...
exports['set instance'] = async function (test) {    
    const result = await setinstance.execute([ 'counter', addresses[0] ]);
    
    test.equal(result, addresses[0]);
    
    const config = configs.loadConfiguration();

    test.equal(config.instances.counter, addresses[0]);
